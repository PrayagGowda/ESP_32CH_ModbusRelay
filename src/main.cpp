...
const char* SSID   = "Airtel_kavi_2209";   
const char* PASS   = "COWORKS@123";
const char* BROKER = "3.6.38.12";
const char* TOKEN  = "0RVWgP5MsdjCFeVsuv70";  
...
String getRoomName(int id) {
  ...
  if (xSemaphoreTake(prefsMutex, portMAX_DELAY)) {
    prefs.begin("cfg", true);              // NVS STORAGE namespace
...
struct MqttMsg {
  char topic[64];           // Stores MQTT topic string
  char payload[256];        // Stores MQTT payload string
};
...
uint16_t modbusCRC(uint8_t *buf, int len) {
  ...
  for (int pos = 0; pos < len; pos++) {      // Process each byte one by one
    ...
    for (int i = 0; i < 8; i++) {     // Each byte has 8 bits, so loop 8 times
      if (crc & 0x0001) {             // Check if least significant bit(LSB) = 1
...
void buildReadCmd(uint8_t channel, uint8_t *cmd) {
  ...
  cmd[2] = 0x00;   // ADDRESS HIGH BYTE
  cmd[3] = channel;
  cmd[4] = 0x00;
  cmd[5] = 0x01;
  uint16_t crc = modbusCRC(cmd, 6);
  cmd[6] = crc & 0xFF;       // Stores LOW byte first
  cmd[7] = (crc >> 8) & 0xFF;  // Stores HIGH byte
}
...
delay(1);
  ...
  if (printLog) {
  Serial.print("RX: ");
  for (int i = 0; i < 8; i++) Serial.printf("%02X ", resp[i]);
  Serial.println();
}
  return 1; // success
}
...
void sendTelemetry(int roomId = 0, String rawColor = "", String rawState = "", bool isBoot = false) {
  char buf[768];   String colorOut;    bool stateBool;
...
if (data["device_config"].is<bool>() && data["device_config"] == true) {
  ...
  if (xSemaphoreTake(prefsMutex, portMAX_DELAY)) {
    ...
    String clientId = prefs.getString("mqttClientId", "");  
    int mqttPort    = prefs.getInt("mqttPort", MQTT_PORT);
...
if (data["ota"].is<JsonObject>()) {
  ...
  int newPort = data["mqttPort"] | MQTT_PORT;
...
if (newSSID.length() > 0 && newPASS.length() > 0) {
  ...
  if (newClientId.length() > 0)
        prefs.putString("mqttClientId", newClientId);   
         prefs.putInt("mqttPort", newPort);   
        prefs.putBool("mqtt_updated", true);
       configChanged = true;
     }
       prefs.end();
    xSemaphoreGive(prefsMutex);
...
if (sdAvailable && configChanged) {
  ...
  devDoc["mqtt"]["clientId"] = prefs.getString("mqttClientId", ""); 
  devDoc["mqtt"]["port"] = prefs.getInt("mqttPort", MQTT_PORT);    
  prefs.end();
...
for (JsonPair kv : root) {
  ...
  for (int i=0;i<8;i++){ char t[4]; sprintf(t,"%02X ",cmd[i]); g_tx+=t; }
  sendCommandDebug(cmd, resp, len);
  for (int i=0;i<len;i++){ char t[4]; sprintf(t,"%02X ",resp[i]); g_rx+=t; }

  // RED
  buildReadCmd(r.red, cmd);
  for (int i=0;i<8;i++){ char t[4]; sprintf(t,"%02X ",cmd[i]); r_tx+=t; }
  sendCommandDebug(cmd, resp, len);
  for (int i=0;i<len;i++){ char t[4]; sprintf(t,"%02X ",resp[i]); r_rx+=t; }

  // ORANGE
  buildReadCmd(r.orange, cmd);
  for (int i=0;i<8;i++){ char t[4]; sprintf(t,"%02X ",cmd[i]); o_tx+=t; }
  sendCommandDebug(cmd, resp, len);
  for (int i=0;i<len;i++){ char t[4]; sprintf(t,"%02X ",resp[i]); o_rx+=t; }

  // BUZZER
  buildReadCmd(r.buzzer, cmd);
  for (int i=0;i<8;i++){ char t[4]; sprintf(t,"%02X ",cmd[i]); b_tx+=t; }
  sendCommandDebug(cmd, resp, len);
  for (int i=0;i<len;i++){ char t[4]; sprintf(t,"%02X ",resp[i]); b_rx+=t; }
  ...
  "\"req1\":{\"green\":{\"req\":\"%s\",\"res\":\"%s\"}},"
  "\"req2\":{\"red\":{\"req\":\"%s\",\"res\":\"%s\"}},"
  "\"req3\":{\"orange\":{\"req\":\"%s\",\"res\":\"%s\"}},"
  "\"req4\":{\"buzzer\":{\"req\":\"%s\",\"res\":\"%s\"}}"
  
  "}}}",
  getTimestampMs(),
  roomId,
  roomName.c_str(),
  g_tx.c_str(), g_rx.c_str(),
  r_tx.c_str(), r_rx.c_str(),
  o_tx.c_str(), o_rx.c_str(),
  b_tx.c_str(), b_rx.c_str()
);
...
if (xSemaphoreTake(prefsMutex, portMAX_DELAY)) {
  ...
  savedClientId = prefs.getString("mqttClientId", "");
  savedPort     = prefs.getInt("mqttPort", MQTT_PORT);
  prefs.end();
  xSemaphoreGive(prefsMutex);
}

mqtt.setServer(savedBroker.c_str(), savedPort);
...
void mqttTask(void *pv) {
  String deviceId = getDeviceId();        // Reads ESP32 MAC address and converts to unique ID.
...
bootSnap.captured = true;
  ...
  for (;;) {
    ...
    mqtt.loop();   //  keepalive , no mutex needed — only mqttTask touches mqtt
...
void buttonTask(void *pv) {
  while (1) {
    ...
    checkButton(BUTTON4_PIN, 4);    
    checkButton(BUTTON5_PIN, 5);      checkButton(BUTTON6_PIN, 6);  
...
bool loadConfigFromSD() {
  ...
  if (!devFile) {
    ...
    if (newFile) {
      JsonDocument defDoc;
      defDoc["wifi"]["ssid"]        = "Airtel_kavi_2209";          defDoc["wifi"]["pass"]  = "COWORKS@123";
      defDoc["mqtt"]["server"]      = "3.6.38.12";    defDoc["mqtt"]["token"]  = "0RVWgP5MsdjCFeVsuv70";
    ...
    if (!err) {
      ...
      if (devDoc["mqtt"].is<JsonObject>()) {
        ...
        String clientId = devDoc["mqtt"]["clientId"] | "";       
        int port = devDoc["mqtt"]["port"] | MQTT_PORT;
        prefs.begin("cfg", false);
...
void setup() {
  ...
  sdAvailable = SD.begin(13);  // CS pin
  ...
  pinMode(BUTTON5_PIN, INPUT_PULLUP);     pinMode(BUTTON6_PIN, INPUT_PULLUP);
   //pinMode(BUTTON7_PIN, INPUT_PULLUP);      pinMode(BUTTON8_PIN, INPUT_PULLUP);

  pinMode(BOOST_ENABLE_PIN, OUTPUT);
  digitalWrite(BOOST_ENABLE_PIN, HIGH);    // Turns ON onboard boost converter for RS485 

  pinMode(RS485_RE_PIN, OUTPUT);
  digitalWrite(RS485_RE_PIN, LOW);        // Controls RS485 receive/transmit mode. LOW = receive, HIGH = transmit

  pinMode(RS485_SHUTDOWN_PIN, OUTPUT);
  digitalWrite(RS485_SHUTDOWN_PIN, HIGH);   //Enables RS485 transceiver chip (LOW = shutdown, HIGH = active)
  ...
  relayQueue = xQueueCreate(15, sizeof(RelayCmd));     // Can store relay commands
  prefsMutex = xSemaphoreCreateMutex();     
  if (relayQueue == NULL) {
    Serial.println("[ERROR] Queue creation failed!");     // checks whether the RTOS queue was created or not
  }
...
for (int i = 1; i <= ncr; i++){
  ...
  if (!prefs.isKey(key.c_str())) {
    prefs.putString(key.c_str(), "Room_" + String(i));  // Creates default room names if not present
  }
}
prefs.end();
